	...
	_onTrigger() {
		...
		this.loadData(this.URL, data)
			.then(data => {
				...
				if (this.URL == Constants.URL_READ_COMMENT_LIST || this.URL == Constants.URL_READ_BESTCOMMENT_LIST) {
					$(document).trigger('fillcomment');
				}
			})
			.catch(e => {
				console.log(e);
...
commentInfinity.prototype = infiniteScroll;
